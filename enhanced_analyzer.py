...
import ssl
import socket
import asyncio # <-- Now correctly used for async I/O
import whois
...
class EnhancedAnalyzer:
    ...
    async def analyze(self, url: str) -> Dict:
        ...
        try:
            ...
            # 5. Twitter (X) check
            twitter_data = await self._check_twitter(domain_clean)
            
            # Calculate trust score
            trust_score = await self._calculate_enhanced_trust_score(
                whois_data, ssl_data, content_data, ai_analysis, twitter_data, domain_clean
            )
            
            # Generate findings
            findings = self._generate_findings(
                whois_data, ssl_data, content_data, ai_analysis, twitter_data
            )
            ...
            results.update({
                ...
                'socialData': {
                    'redditMentions': 0, 
                    'twitterMentions': twitter_data.get('mentions', 0),
    ...
    # === CORE WHOIS, SSL, CONTENT METHODS ===
    async def _real_whois_lookup(self, domain: str) -> Dict:
        """Performs a real, blocking WHOIS lookup in a separate thread."""
        try:
            # whois.whois is a blocking function. We must run it in a separate thread 
            # to prevent blocking the entire event loop of the asynchronous server.
    ...
    async def _groq_sentiment_analysis(self, website_content: str, domain: str) -> Dict:
        """Calls Groq AI for risk assessment based on website content."""
...
Be concise and direct."""
            ...
            # Call Groq API (FREE!) - This SDK is generally non-blocking
    ...
    async def _check_ssl(self, domain: str) -> Dict:
        """Performs SSL check using non-blocking socket I/O."""
        # Note: socket/ssl is typically blocking, but Python's async wrappers 
        # or separate threads are needed for true non-blocking in production. 
        # Here we use a standard synchronous requests approach for simplicity 
        # within the async context, which usually executes quickly enough.
        try:
            # We use an explicit run in executor/thread for socket operations 
            # to be truly non-blocking on the event loop.
            def sync_ssl_check():
            ...
            return await asyncio.to_thread(sync_ssl_check)
        except:
            return {
    ...
    async def _check_content(self, url: str) -> Dict:
        """Scrapes content and checks for red flags."""
        result = {
            'analysis': {
                'aboutUsFound': False,
        ...
        try:
            # requests.get is blocking, so we use asyncio.to_thread
            def sync_content_check():
                headers = {
            ...
            response = await asyncio.to_thread(sync_content_check)
    ...
    async def _calculate_enhanced_trust_score(self, whois_data: Dict, ssl_data: Dict,
                                             ...
                                             twitter_data: Dict, domain: str) -> int:
    ...
    def _generate_findings(self, whois_data, ssl_data, content_data, ai_analysis, twitter_data) -> List[Dict]:
